...
session_start();
  ...
  include 'php/conn.php';
    ...
    $qu3 = "SELECT SUM(prix_formation) as prix1 FROM users, formation WHERE situation = 'Réglé' AND (date_inscription >= DATE_FORMAT(NOW() ,'%Y-%m-01')) OR (date_reinscription >= DATE_FORMAT(NOW() ,'%Y-%m-01')) ";
          ...
          while($row1 = mysqli_fetch_assoc($res1)){
            while($row2 = mysqli_fetch_assoc($res2)){
              while($row3 = mysqli_fetch_assoc($res3)){
                $b = ($row1["prix1"] - ( $row2["prix2"] + ($row3["prix1"]/2) ));
                $json2[] = $b ;
              }
            }
          }
  ...
  <script type="text/javascript">
    ...
    var config2 = {
      ...
      options: {
        ...
        title: {
          display: true,
          text: 'Graphe Bénefices sur 12 mois'
        },
        tooltips: {
          mode: 'index',
          intersect: false,
                  ...
                  <div class="row">
                    <div class="col">
                      ...
                      <?php
                        ...
                        $qu = "SELECT SUM(prix_formation) as prix FROM users WHERE situation = 'Réglé' AND date_inscription >= DATE_FORMAT(NOW() ,'%Y-%m-01')";
                        $res = mysqli_query($conn,$qu);
                        $resChe = mysqli_num_rows($res);
                        if($resChe > 0){
                          while($row = mysqli_fetch_assoc($res)){
        ...
        <div class="col-xl-6">
          <div class="card">
            <div class="card-header border-0">
              <div class="row align-items-center">
                <div class="col">
                  <h3 class="mb-0">Bénefices <span style="font-size: 10px">sur 12 mois</span></h3>
                </div>
              </div>
            </div>
            <div class="table-responsive">
              ...
              <table class="table align-items-center table-flush">
                ...
                <tbody id="refresh1">
                <?php
                    $qu1 = "SELECT  date_inscription, SUM(prix_formation) as prix1 FROM users WHERE situation = 'Réglé' AND (date_inscription >= DATE_FORMAT(NOW() ,'%Y-%m-01')) OR (date_reinscription >= DATE_FORMAT(NOW() ,'%Y-%m-01')) ";
                    $qu2 = "SELECT date_charge, SUM(prix) as prix2 FROM charge WHERE date_charge >= DATE_FORMAT(NOW() ,'%Y-%m-01')";
                    $qu3 = "SELECT date_inscription, SUM(prix_formation) as prix1 FROM users WHERE situation = 'Réglé' AND (date_inscription >= DATE_FORMAT(NOW() ,'%Y-%m-01')) OR (date_reinscription >= DATE_FORMAT(NOW() ,'%Y-%m-01')) ";
                                ...
                                while($row3 = mysqli_fetch_assoc($res3)){
                                   for ( $i=1;$i<13;$i++){
                                    echo '
                                      ...
                                      $date_inscription1 = $row1["date_inscription"];
                                      $date_charge = $row2["date_charge"];

                                      $d1 = strtotime("$date_inscription1");
                                      $d2 = strtotime("$date_charge");

                                      $m1 = date("m", $d1);
                                      $m2 = date("m", $d2);
                                      
                                      if ( $m1 == $i && $m2 == $i ){
                                        echo '<td class="text-success">'.($row1["prix1"] - ( $row2["prix2"] + ($row3["prix1"]/2) )).'</td>';
                                      }
                                    echo '</tr>';  
                                    }
                                }
