          ...
          while($row1 = mysqli_fetch_assoc($res1)){
            while($row2 = mysqli_fetch_assoc($res2)){
              while($row3 = mysqli_fetch_assoc($res3)){
                $b = ($row1["prix1"] - ( $row2["prix2"] + $row3["prix3"] ));
                $json2[] = $b ;
              }
            }
          }
          ...
          <div class="card">
            ...
            <div class="table-responsive">
              ...
              <table class="table align-items-center table-flush">
                ...
                <?php
                   for ( $i=1;$i<13;$i++){
                    $qu1 = "SELECT  date_inscription, SUM(prix_formation) as prix1 FROM users WHERE situation = 'Réglé' AND date_inscription between DATE_FORMAT(NOW() ,'%Y-$i-01') and DATE_FORMAT(NOW() ,'%Y-$i-30') OR date_reinscription between DATE_FORMAT(NOW() ,'%Y-$i-01') and DATE_FORMAT(NOW() ,'%Y-$i-30') ";
                    $qu2 = "SELECT date_charge, SUM(prix) as prix2 FROM charge WHERE date_charge between DATE_FORMAT(NOW() ,'%Y-$i-01') and DATE_FORMAT(NOW() ,'%Y-$i-30')";
                    $qu3 = "SELECT date_inscription, SUM(prix) as prix3 FROM formateur WHERE situation = 'Réglé' AND date_inscription between DATE_FORMAT(NOW() ,'%Y-$i-01') and DATE_FORMAT(NOW() ,'%Y-$i-30') ";
                              ...
                              while($row2 = mysqli_fetch_assoc($res2)){
                                while($row3 = mysqli_fetch_assoc($res3)){
                                    echo '
                                      <tr class="text-center">
                                        <th scope="row">'.$i.'</th>
                                        <td class="text-success">'.($row1["prix1"] - ( $row2["prix2"] + $row3["prix3"] )).'</td>
                                      </tr>';  
                                    }
                                }
                              }
