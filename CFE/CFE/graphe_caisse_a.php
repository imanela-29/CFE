            ...
            <div class="table-responsive">
              ...
              <table class="table align-items-center table-flush">
                ...
                <tbody id="">
                  <?php
                    for ( $i=1;$i<13;$i++){
                    $qu = "SELECT id, date_inscription, SUM(prix_formation) as total FROM users WHERE situation = 'Réglé' AND date_inscription between DATE_FORMAT(NOW() ,'%Y-$i-01') and DATE_FORMAT(NOW() ,'%Y-$i-30') GROUP BY date_inscription ORDER BY date_inscription DESC";
                    $res = mysqli_query($conn,$qu);
                    $resChe = mysqli_num_rows($res);
                    if($resChe > 0){
                      while($row = mysqli_fetch_assoc($res)){
